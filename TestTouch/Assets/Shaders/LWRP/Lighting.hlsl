...
half4 LightweightFragmentBlinnPhong(InputData inputData, half3 diffuse, half4 specularGloss, half shininess, half3 emission, half alpha , half3 rim)
{
	Light mainLight = GetMainLight();
	mainLight.attenuation *= MainLightRealtimeShadowAttenuation(inputData.shadowCoord);
	MixRealtimeAndBakedGI(mainLight, inputData.normalWS, inputData.bakedGI, half4(0, 0, 0, 0));
...
#endif
	half3 fullDiffuse = diffuseColor + inputData.vertexLighting;
	half3 finalColor = fullDiffuse * diffuse + emission + rim;
...
//ABOUT HAIR ALGORITHM
half LWRP_StandSpec(half3 L ,half3 V , half3 T , half range , half strength) {
	//H 
	//http://amd-dev.wpengine.netdna-cdn.com/wordpress/media/2012/10/Scheuermann_HairRendering.pdf
	half3 h = normalize(L + V);
	//sinTH
	half dotTH = dot(T, h);
	half th = sqrt(1 - (dotTH*dotTH));
	th = pow(th, range);
	th *= strength;
	//ditAtten
	half dit;
	dit = smoothstep(-1, 0, dot(T, h));

	return dit * th;
}
half3 ShiftTangent(half3 N , half3 T , half shift){
	half3 shiftT = N*shift + T;
	return normalize(shiftT);
}
half4 LightweightFragmentHair(InputData inputData, half3 diffuse, half4 specularGloss, half shininess, half3 emission, half alpha, half3 rim,half3 tangentWS,half range, half strength, half4 haircolor)
{
	Light mainLight = GetMainLight();
	mainLight.attenuation *= MainLightRealtimeShadowAttenuation(inputData.shadowCoord);
	MixRealtimeAndBakedGI(mainLight, inputData.normalWS, inputData.bakedGI, half4(0, 0, 0, 0));
	...
	//HAIR
	half hairRefl1 = LWRP_StandSpec(mainLight.direction, inputData.viewDirectionWS, tangentWS, range, strength);
	half hairRefl2 = LWRP_StandSpec(mainLight.direction, inputData.viewDirectionWS, tangentWS, range / 16, strength / 2.0);
	half hairRefl = hairRefl1 + hairRefl2;
...
#endif
	half3 fullDiffuse = diffuseColor + inputData.vertexLighting;
	half3 finalColor = fullDiffuse * diffuse + emission + rim + hairRefl* haircolor;
